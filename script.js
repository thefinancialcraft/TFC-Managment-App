...
function loopTypeEffect(element, greetText, nameText) {
    element.innerHTML = '';
    var i = 0;
    var j = 0;
    var state = 'greet';
    ...
    function type() {
        if (state === 'greet') {
            if (i < greetText.length) {
                element.innerHTML += greetText.charAt(i);
                i++;
                setTimeout(type, 100); // Adjust the speed here (lower number for faster typing)
            } else {
                setTimeout(() => {
                    element.innerHTML = '';
                    i = 0;
                    state = 'name';
                    setTimeout(type, 500); // Short delay before typing the name
                }, 1000); // Delay before removing the greeting text
            }
        } else if (state === 'name') {
            if (j < nameText.length) {
                element.innerHTML += nameText.charAt(j);
                j++;
                setTimeout(type, 100); // Adjust the speed here (lower number for faster typing)
            } else {
                setTimeout(() => {
                    ...
                    j = 0;
                    state = 'greet';
                    setTimeout(type, 500); // Delay before re-typing starts
                }, 1000); // Delay before removing the name text
            }
        }
    }
